...
import { ArgTypes, Controls, Meta, Source } from '@storybook/blocks'
...
<span type="info">
  When you are using a custom app (self-hosted), you need to provide the
...
Here below a simple example on how to to access the current mode (`test` | `live`) from the `settings` object:
